#!/usr/bin/env node
/** Offline verification planning. No network/model/external mutation. */
...
import { buildOpportunityVerificationPlans } from "./verification-plan.js";
import { JsonlOpportunityVerificationResolutionStore } from "./verification-resolutions.js";

const HELP = `Prepare verification plans offline

Usage:
  node --import tsx src/opportunities/verification-plan-cli.ts [options]
...
Options:
      ...
      --limit <n>                 max verification plans (default: 25)
...
This command reads local persisted state and derives verification status only.
It does not fetch Reddit, call a model, contact anyone, claim/accept work, submit,
hire, move money, or write a verification resolution.
`;
...
async function main(): Promise<void> {
  ...
  const resolutionFile = resolve(
    values["resolution-file"] ?? join(config.stateRoot, "opportunity-verifications.jsonl"),
  );
  ...
  const resolutionStore = new JsonlOpportunityVerificationResolutionStore(resolutionFile);
  const resolutions = await resolutionStore.list();
  const plans = buildOpportunityVerificationPlans(dossiers, resolutions, limit);
  const allCheckStates = plans.flatMap((plan) => plan.checks.map((check) => check.state));
  const allCheckKinds = plans.flatMap((plan) => plan.checks.map((check) => check.kind));

  const output = {
    ok: true,
    mode: "offline-verification-plan",
    profile: profile.id,
    ...
    resolutionRecordCount: resolutions.length,
    counts: {
      byPlanState: countBy(plans.map((plan) => plan.state)),
      byCheckState: countBy(allCheckStates),
      byCheckKind: countBy(allCheckKinds),
    },
    plans,
  };
  ...
  process.stdout.write(
    [
      `Verification plans: ${String(plans.length)}`,
      `profile: ${profile.id}`,
      `evaluator: ${evaluatorId ?? "latest current evaluation"}`,
      ...plans.map(
        (plan, index) =>
          `${String(index + 1)}. [${plan.state}] ${plan.opportunityId} — resolved ${String(plan.counts.resolved)}/${String(plan.checks.length)}, external ${String(plan.counts.requires_external_verification)}`,
      ),
    ].join("\n") + "\n",
  );
}
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`verification plan preparation failed: ${message}\n`);
  process.exitCode = 1;
});
