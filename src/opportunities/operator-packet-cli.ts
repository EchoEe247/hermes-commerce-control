#!/usr/bin/env node
/** Offline operator-preparation packets. No network/model/external mutation. */
...
const HELP = `Prepare operator-facing opportunity packets offline

Usage:
  node --import tsx src/opportunities/operator-packet-cli.ts [options]
...
Options:
      ...
      --max-age-hours <n>       non-rejected listing/evaluation age window (default: ${String(DEFAULT_RANKING_MAX_AGE_HOURS)}; 0 disables)
      --as-of <timestamp>       deterministic ranking clock for replay/tests
      --limit <n>               max preparation packets (default: 25)
...
This command only prepares analysis for an operator. It does not fetch Reddit,
...
async function main(): Promise<void> {
  ...
  const packets = prepareOpportunityOperatorPackets(ranked, limit);
  const output = {
    ok: true,
    mode: "offline-operator-preparation",
    profile: profile.id,
    ...
    counts: {
      byReadiness: countBy(packets.map((packet) => packet.readiness)),
      byOperatorAction: countBy(packets.map((packet) => packet.ranking.operatorAction)),
      byExecutionRoute: countBy(packets.map((packet) => packet.ranking.executionRoute)),
    },
    packets,
  };
  ...
  process.stdout.write(
    [
      `Operator preparation: ${String(packets.length)} packet(s)`,
      `profile: ${profile.id}`,
      `evaluator: ${evaluatorId ?? "latest current evaluation"}`,
      ...packets.map(
        (packet, index) =>
          `${String(index + 1)}. [${String(packet.ranking.score)} ${packet.ranking.operatorAction}/${packet.ranking.executionRoute} ${packet.readiness}] ${packet.opportunity.title}`,
      ),
    ].join("\n") + "\n",
  );
}
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`operator packet preparation failed: ${message}\n`);
  process.exitCode = 1;
});
